...
// 构造函数是不需要返回值的，使用new来创建对象时，如果return的是非对象类型，会忽略返回值，如果return的是对象，则返回该对象（若return null也会忽略返回值）
...
//   })(i)
// }
// let 每次循环会生成一个封闭的作用域和setTimeout绑定，var每次循环会覆盖上一次的作用域
...
// 3. 使用for in 会遍历数组所以可枚举属性，包括原型链，所以for in 更适合遍历对象

// for of
// for in 遍历的是数组的索引，for of遍历的是数组的元素
// for of 遍历的只是数组的元素，而不包括数组原型属性和索引
